...
export async function buscarChamadoPorNumero({ escolaId, ticketNumber, ticketCode, uid }) {
    if (!escolaId) throw new Error("escolaId obrigatorio");
    ...
    if (ticketCode) {
        constraints.push(where("codigoChamado", "==", ticketCode));
    } else {
        constraints.push(where("numeroChamado", "==", Number(ticketNumber)));
    }

    // Se passar UID (visitante/usuario comum), filtra para garantir permissão
    if (uid) {
        constraints.push(where("criadoPorUid", "==", uid));
    }
...
export function ouvirComentarios(chamadoId, callback) {
    ...
    return onSnapshot(q, (snap) => {
        const itens = snap.docs.map((d) => ({ id: d.id, ...d.data(), _tipoItem: "comentario" }));
        callback(itens);
    });
}
...
export function ouvirAtualizacoes(chamadoId, callback) {
    ...
    return onSnapshot(q, (snap) => {
        const itens = snap.docs.map((d) => ({ id: d.id, ...d.data(), _tipoItem: "atualizacao" }));
        callback(itens);
    });
}
